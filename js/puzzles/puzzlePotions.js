...
export async function mount({onSolved,onFail,meta}) {
  ...
  ingredients.forEach(it=>{
    ...
    d.draggable=true; d.textContent=it.label; d.dataset.val=it.id;
  ...
  overlay.content.appendChild(beaker);
  ...
  ok.addEventListener("click", ()=>{
    ...
    if(seq.join(",") === sol){
      playActionEffect("collect","potion");
      onSolved({score:120});
      cleanup();
      startNextMiniGame();
    } else {
      playActionEffect("error");
      alert("Mauvais mélange");
      onFail({penalty:20});
    }
  });
...
function makeOverlay(title, subtitle){ 
  ...
  const box=document.createElement("div"); box.className="puzzle-container"; box.style.width="480px";
